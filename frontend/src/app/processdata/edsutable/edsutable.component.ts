...
export class EdsutableComponent implements OnInit {
  ...
  constructor(public pds: ProcessDataService, private ps: ProjectService, private dataService: DataService) { 
  ...
  public getItemColor(item, shiftKey : boolean) {
    return item.PSU != null && item.PSU == this.pds.selectedPSU ? 'rgb(255,192,203)' : 'white'
  }
  ...
  async onItemClick(item, shiftDown) {
    ...
    this.pds.selectedPSU = this.pds.selectedPSU; // propagate a fake selection, so that the map will update the style indexing based on selected psu - and rerender
  }
}
